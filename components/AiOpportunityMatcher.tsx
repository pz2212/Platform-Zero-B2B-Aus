...
import { Camera, Upload, ScanLine, CheckCircle, Send, MessageSquare, AlertCircle, Loader2, Image as ImageIcon, FolderOpen, X, Store, MapPin, Share2, Heart, Edit2, ChevronDown, Package, Check, Sparkles } from 'lucide-react';
...
export const AiOpportunityMatcher: React.FC<AiOpportunityMatcherProps> = ({ user }) => {
  ...
  const handleAddToInventory = () => {
    ...
    const newItem: InventoryItem = {
        ...
        quantityKg: 100, // Default batch size
        status: 'Available',
        ...
        // Save the price set in the scanner
        lastPriceVerifiedDate: new Date().toISOString(),
        discountPricePerKg: price > 0 ? price : product.defaultPricePerKg
    };

    mockService.addInventoryItem(newItem);
    setIsSaved(true);
    
    // Update the product's default price in catalog as well
    if (price > 0) {
        mockService.updateProductPrice(product.id, price);
    }
    
    alert(`${analysisResult.name} successfully logged as an active stock batch!`);
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {analysisResult && (
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-emerald-100 animate-in slide-in-from-left-4 duration-500">
                    <div className="flex justify-between items-start mb-6">
                        ...
                        <div className="flex gap-2">
                             <button 
                                ...
                                className={`p-3 rounded-full transition-all active:scale-95 shadow-md ${isSaved ? 'bg-emerald-600 text-white shadow-emerald-100' : 'bg-white border border-gray-200 text-gray-400 hover:text-emerald-600 hover:border-emerald-200'}`}
                                title="Log this Photo & Batch to Inventory"
                             >
                                <Package size={24} strokeWidth={isSaved ? 2.5 : 2}/>
                             </button>
                             <button 
                    ...
                    <div className="space-y-6">
                        ...
                        {/* NEW: Explicit Add to Inventory Button */}
                        {!isSaved ? (
                            <button 
                                onClick={handleAddToInventory}
                                className="w-full py-5 bg-white border-2 border-emerald-500 text-emerald-600 rounded-[1.5rem] font-black text-xs uppercase tracking-[0.2em] shadow-sm hover:bg-emerald-600 hover:text-white transition-all active:scale-95 flex items-center justify-center gap-3 group"
                            >
                                <Package size={20} className="group-hover:scale-110 transition-transform"/> Log Batch to My Inventory
                            </button>
                        ) : (
                            <div className="w-full py-5 bg-emerald-600 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-xl animate-in zoom-in">
                                <CheckCircle size={20} strokeWidth={3}/> Batch Live in Catalog
                            </div>
                        )}
                    </div>
                </div>
        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-200 flex flex-col h-[700px] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                {/* NEW: QUICK INVENTORY ACTION LINK IN OPPORTUNITIES SIDEBAR */}
                {analysisResult && !isSaved && (
                    <div className="p-5 bg-indigo-50 border-2 border-indigo-100 rounded-2xl flex items-center justify-between animate-in slide-in-from-top-2 duration-500">
                        <div className="flex items-center gap-3">
                            <Sparkles size={18} className="text-indigo-600"/>
                            <p className="text-[11px] font-black text-indigo-900 uppercase">Save batch to sell later?</p>
                        </div>
                        <button 
                            onClick={handleAddToInventory}
                            className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all"
                        >
                            Add to Catalog
                        </button>
                    </div>
