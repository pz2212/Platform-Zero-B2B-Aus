...
const OrderDetailsModal = ({ isOpen, onClose, order, products, customer, isBlocked }: { 
    ...
    const getStatusIndex = (status: string) => {
        switch (status) {
            ...
            case 'Ready for Delivery': return 1;
                ...
                <div className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
                    ...
                    <div className="px-2">
                        <div className="flex justify-between relative">
                            <div className="absolute top-5 left-8 right-8 h-1 bg-gray-100 rounded-full z-0"></div>
                            <div 
                                className="absolute top-5 left-8 h-1 bg-emerald-500 rounded-full z-0 transition-all duration-1000 ease-out"
                                ...
                                return (
                                    <div key={idx} className="flex flex-col items-center relative z-10">
                                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-500 border-4 ${
                                            isComplete 
                                            ? 'bg-emerald-600 border-white text-white shadow-lg shadow-emerald-100' 
                                        ...
                                        {isCurrent && (
                                            <div className="absolute -top-1 w-2 h-2 bg-emerald-400 rounded-full animate-ping"></div>
                                        )}
                                    </div>
                                );
                            })}
