...
import { 
  X, CheckCircle, Send, Loader2, Users, Smartphone, Contact, Plus, MessageCircle, Copy, Link as LinkIcon
} from 'lucide-react';
...
export const ShareModal: React.FC<{
  ...
  const getSmsContent = () => {
    ...
    // Use lot number if available, otherwise fallback to a generic product view
    const referenceId = item.lotNumber === 'CATALOG' ? item.productId : item.id;
    const productLink = generateProductDeepLink('product', referenceId, senderName, priceValue);
    ...
    return {
        ...
        shortLink: productLink // Changed to actual link for the failsafe copy button
    };
  };

  const handleCopyOnly = () => {
      const content = getSmsContent();
      navigator.clipboard.writeText(content.shortLink);
  ...
  const handleSendBlast = () => {
    ...
    // Trigger the native app for the first recipient
    try {
        const primaryNumber = targetNumbers[0] as string;
        triggerNativeSms(primaryNumber, content.text);
    } catch (err) {
        console.error("Native SMS fail", err);
    }
    
    setTimeout(() => {
      alert(`🚀 Dispatch workflow active! Opening native messaging for ${targetNumbers.length} recipients.`);
      setIsSending(false);
      onComplete();
    }, 1200);
  };
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
          <div className="bg-emerald-50 rounded-3xl p-6 border-2 border-emerald-100 relative shadow-sm">
            ...
            <p className="text-sm text-emerald-900 italic leading-relaxed pt-2">
              "{smsContent.text.replace(/http.*$/, '')}<span className="underline font-bold text-emerald-700">https://pz.io/l/...</span>"
            </p>
            <button 
                onClick={handleCopyOnly}
                className="mt-4 flex items-center gap-2 text-[10px] font-black text-emerald-700 uppercase tracking-widest hover:underline"
            >
