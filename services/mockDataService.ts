...
export const USERS_INITIAL: User[] = [
  ...
  { id: 'u2', name: 'Sarah Wholesaler', businessName: 'Fresh Wholesalers Adelaide', role: UserRole.WHOLESALER, email: 'sarah@fresh.com', phone: '0411 111 111', dashboardVersion: 'v2', activeSellingInterests: ['TOMATOES', 'LETTUCE', 'EGGPLANT'], activeBuyingInterests: ['POTATOES', 'APPLES', 'CARROTS'], businessProfile: { isComplete: true, businessLocation: 'Pooraka, SA' } as any, favoriteProductIds: [], isConfirmed: true, hasSetCredentials: true, isStripeConnected: true, stripeAccountId: 'acct_12345' },
  { id: 'u3', name: 'Bob Farmer', businessName: 'Green Valley Farms', role: UserRole.FARMER, email: 'bob@greenvalley.com', phone: '0422 222 222', dashboardVersion: 'v2', activeSellingInterests: ['POTATOES', 'APPLES', 'TOMATOES', 'CARROTS'], activeBuyingInterests: [], businessProfile: { isComplete: true, businessLocation: 'Mildura, VIC' } as any, favoriteProductIds: [], isConfirmed: true, hasSetCredentials: true, isStripeConnected: false },
  { id: 'u4', name: 'Alice Consumer', businessName: 'The Morning Cafe', role: UserRole.CONSUMER, email: 'alice@cafe.com', phone: '0433 333 333', industry: 'Cafe', smsNotificationsEnabled: true, businessProfile: { isComplete: true, businessLocation: 'Adelaide CBD' } as any, favoriteProductIds: ['p1', 'p2'], activeBuyingInterests: ['TOMATOES', 'LETTUCE'], isConfirmed: true, hasSetCredentials: true },
  { id: 'u5', name: 'Gary Grocer', businessName: 'Local Corner Grocers', role: UserRole.GROCERY, email: 'gary@grocer.com', phone: '0444 444 444', industry: 'Grocery Store', smsNotificationsEnabled: true, businessProfile: { isComplete: true, businessLocation: 'Glenelg, SA' } as any, favoriteProductIds: [], activeBuyingInterests: ['POTATOES', 'CARROTS'], isConfirmed: true, hasSetCredentials: true },
];
...
const INITIAL_INVENTORY: InventoryItem[] = [
    { id: 'inv1', ownerId: 'u2', productId: 'p1', lotNumber: 'PZ-LOT-1001', quantityKg: 30, status: 'Available', uploadedAt: new Date().toISOString(), harvestDate: new Date().toISOString(), expiryDate: new Date(Date.now() + 86400000 * 5).toISOString(), warehouseLocation: 'Bin A1', isPublicMarketplace: true, harvestLocation: 'South Australia' },
    { id: 'inv2', ownerId: 'u2', productId: 'p2', lotNumber: 'PZ-LOT-1002', quantityKg: 80, status: 'Available', uploadedAt: new Date().toISOString(), harvestDate: new Date().toISOString(), expiryDate: new Date(Date.now() + 86400000 * 3).toISOString(), warehouseLocation: 'Bin B4', isPublicMarketplace: true, harvestLocation: 'Virginia, SA' },
...
class MockDataService {
  ...
  getMatchedSuppliers(userId: string) {
    const user = this.users.find(u => u.id === userId);
    if (!user || !user.activeBuyingInterests) return [];
    
    const buying = user.activeBuyingInterests.map(i => i.toUpperCase());
    
    return this.users.filter(u => 
      (u.role === UserRole.WHOLESALER || u.role === UserRole.FARMER) && 
      u.id !== userId &&
      u.activeSellingInterests?.some(s => buying.includes(s.toUpperCase()))
    ).map(supplier => ({
        ...supplier,
        matches: supplier.activeSellingInterests?.filter(s => buying.includes(s.toUpperCase())) || []
    }));
  }
  ...
  connectStripe(userId: string) {
      ...
      if (user) {
          ...
          user.stripeAccountId = `acct_${Math.random().toString(36).substring(7)}`;
          this.saveToStorage();
      }
  }

  publishToMarketplace(inventoryId: string, discountPrice: number, minOrder: number, logisticsPricePerKg: number, minLogisticsKg: number) {
      const item = this.inventory.find(i => i.id === inventoryId);
      if (item) {
          item.isPublicMarketplace = true;
          item.discountPricePerKg = discountPrice;
          item.minOrderQuantity = minOrder;
          item.logisticsPricePerKg = logisticsPricePerKg;
          item.minLogisticsKg = minLogisticsKg;
          // For legacy compatibility where only a flat logistics price might be checked
          item.logisticsPrice = logisticsPricePerKg * minLogisticsKg;
          this.saveToStorage();
      }
  }
  ...
  createFullOrder(buyerId: string, items: any[], total: number, sellerId: string = 'u2') {
    const order: Order = {
      id: `o-${Date.now()}`,
      buyerId,
      sellerId, 
  ...
  updateUserInterests(id: string, selling: string[], buying: string[]) {
    ...
    if (u) {
      ...
      if (buying.length > 0) {
        const matches = this.getMatchedSuppliers(id);
        if (matches.length > 0) {
            this.addAppNotification(
                id,
                'Smart Match Found!',
                `We found ${matches.length} suppliers offering the products you need. Check Supplier Market.`,
                'SYSTEM'
            );
        }
      }
  ...
  updateProcurementQuote(requestId: string, offeredPrice: number) {
    const req = this.procurementRequests.find(r => r.id === requestId);
    if (req) {
      req.status = 'QUOTED';
      req.offeredPrice = offeredPrice;
      this.addAppNotification(
        req.buyerId,
        'Price Quote Received',
        `${this.users.find(u => u.id === req.supplierId)?.businessName} has quoted $${offeredPrice}/${req.unit} for ${req.productName}.`,
        'PRICE_REQUEST'
      );
      this.saveToStorage();
    }
  ...
  acceptProcurementQuote(requestId: string) {
    ...
    if (req) {
      req.status = 'ACCEPTED';
      const total = req.quantity * (req.offeredPrice || 0);
      const order: Order = {
        id: `o-proc-${Date.now()}`,
        buyerId: req.buyerId,
        sellerId: req.supplierId,
        items: [{ productId: req.productId, quantityKg: req.quantity, pricePerKg: req.offeredPrice || 0, unit: req.unit }],
        totalAmount: total,
        status: 'Pending',
        date: new Date().toISOString(),
        paymentStatus: 'Unpaid',
      ...
      this.addAppNotification(
        ...
        `${req.buyerName} has accepted your quote for ${req.productName}. An order has been created.`,
        'ORDER'
      );
      this.saveToStorage();
    }
  ...
  updateCustomerTier(id: string, tier: 'PLATINUM' | 'GOLD' | 'SILVER') {
    const c = this.customers.find(cust => cust.id === id);
    if (c) {
      c.inventoryTier = tier;
      this.saveToStorage();
    }
  }
